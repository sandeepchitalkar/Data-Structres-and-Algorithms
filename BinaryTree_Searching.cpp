//Binary Tree Implementaiton,  BFS Traversal and Searching operation
#include<iostream>
#include<queue>
using namespace std;
	...
	do
	{
		...
		switch(ch)
		{
			...
			case 2:b.BFStraverse(); // DFS traversal of Binary Tree
			       break;
			case 3:
			       cout<<"\n Enter the data to be search in BT";
			       cin>>key;
                   flag = b.search(b.root, key);
                    if (flag == 1) 
                        cout<<"Key "<<key<<" is found in the BT.\n";
                    else 
                        cout<<"Key "<<key<<" is not found in the BT.\n";    
...
int BT:: search(node* temp, int value) {
	...
	search(temp->left, value); // Traverse the left subtree
	search(temp->right, value);// Traverse the right subtree
	}
